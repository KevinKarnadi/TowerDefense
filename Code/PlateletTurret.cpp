...
const int PlateletTurret::Price = 60;
const int PlateletTurret::hp = 8;

PlateletTurret::PlateletTurret(float x, float y) :
	Turret("play/turret-2.png", x, y, Price, 2, hp)
{
...
void PlateletTurret::CreateBullet()
{
	...
	getPlayScene()->BulletGroup->AddNewObject(new PockyBullet(Position + normalized * 36 + normal * 6, diff, rotation, this));
	getPlayScene()->EffectGroup->AddNewObject(new ShootEffect(Position.x + normalized.x * 46 - normal.x * 6, Position.y + normalized.y * 50 - normal.y * 6));
	//getPlayScene()->BulletGroup->AddNewObject(new DirtyEffect("play/dirty-1.png", diff, rotation, this));
