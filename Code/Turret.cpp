...
void Turret::Update(float deltaTime)
{
	...
	if (!Target)
	{
		...
		for (auto& it : scene->EnemyGroup->GetObjects())
		{
			if (this->price == 50)
			{
				if (it->Position.y >= Position.y && it->Position.y < Position.y + scene->BlockSize)
				{
...
void Turret::Draw() const
{
	...
	Sprite::Draw();
	if(price == 100)
		al_draw_circle(Position.x, Position.y, 2.25 * PlayScene::BlockSize, al_map_rgb(255, 0, 0), 2);
...
void Turret::Hit(float damage)
{
	...
	if (hp <= 0) {
		...
		for (auto& it : lockedEnemies)
			it->Target = nullptr;
		scene->ModifyReadMapTiles(Position.x, Position.y);
...
void Turret::Freeze()
{
	//Engine::LOG(Engine::DEBUGGING) << "			FREEZE";
	frozen_time = al_get_time();
	if (Frozen == false)
	{
		coolDown *= 2;
		reload = coolDown;
